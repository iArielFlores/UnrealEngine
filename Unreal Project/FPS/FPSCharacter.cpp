...
void AFPSCharacter::Tick(float DeltaTime)
{
	...
	FVector ray_start = FP_Gun->GetComponentLocation();
	
	FVector ray_forward = FP_Gun->GetRightVector();
	
	FVector ray_end = ray_start + ray_forward * lineDistance;

	DrawDebugLine(GetWorld(), ray_start, ray_end, FColor::Blue);

	
	FHitResult hit; // this will hold all the information from the line trace
	FCollisionQueryParams CollisionParams; // just something we have to set up

	if (GetWorld()->LineTraceSingleByChannel(hit, ray_start, ray_end, ECC_Visibility, CollisionParams)) // do raycast
	{
		if (hit.bBlockingHit) // if our raycast hit a wall
		{
			FVector impactNormal = hit.Normal;
			FVector impactPoint = hit.ImpactPoint;
			

			GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, FString::Printf(TEXT("Hit Location: %f, %f, %f "), impactPoint.X, impactPoint.Y, impactPoint.Z));

			//ray_end - 2 * (impactPoint, ray_end) * impactPoint;

			FVector reflected = ray_end - 2 * FVector::DotProduct(impactNormal, ray_end) * impactNormal;

			DrawDebugLine(GetWorld(), impactPoint, impactPoint + reflected * lineDistance, FColor::Blue);
		}
	}



	//GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Emerald, TEXT(""));
	...
	//FVector LookDirection = playerForward;

	//LookDirection.Normalize();

	//FVector PlayerToSensor = end - start;

	//PlayerToSensor.Normalize();

	//float dot = FVector::DotProduct(LookDirection, PlayerToSensor);

	//GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Blue, FString::Printf(TEXT("Dot: %f"), dot), true);
	//if (dot < precise)
...
FVector AFPSCharacter::GetLocation()
{
	//return GetActorLocation();
	return FP_Gun->GetComponentLocation();
}

FVector AFPSCharacter::GetForward()
{
...
FVector AFPSCharacter::GetSensorLocation()
{
	return sensor->GetActorLocation();
}
FVector AFPSCharacter::GetSensorForward()
{
